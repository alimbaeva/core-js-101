...
function isLeapYear(date) {
  ...
  return new Date(year1, 1, 29).getMonth() === 1;
  // throw new Error('Not implemented');
}
...
function timeSpanToString(startDate, endDate) {
  ...
  const miliSec = time.toString().slice(-3);
  return `${hours.toString().padStart(2, '0')}:${min.toString().padStart(2, '0')}:${sec.toString().padStart(2, '0')}.${miliSec}`;
  // throw new Error('Not implemented');
}
...
function angleBetweenClockHands(date) {
  ...
  const angle = 0.5 * (60 * hour - 11 * min);
